 ...
 // This state will store the parsed data
  ...
  const handleName = (e: React.ChangeEvent<any>) => {
    ...
    if (spaceIndex == -1) {
      ...
      setNameFormated(`${e.target.value.substr(spaceIndex + 1)}, ${e.target.value.substring(0, spaceIndex)}`);
    }
  ...
  const handleFromCountries = (e: string) => {
    ...
    const cityList: JSX.Element[] = [<option key={0} value={0} disabled>Select City</option>];
    
    countrySelected!.cities!.map((city, key) => (
      cityList.push(<option key={key} title="" value={city}> {city} </option>)
    ));
    setCityItems(cityList!);
  };

  const handleFromCity = (e: string) => {
    const citySelected = fromCities!.find(
      (citySelected) => citySelected === e
    );
...
const outputContent = <div>
  ...
  <Form.Item >
    <Input.Group compact>
      <Table style={{width:"50%"}} columns={columns} dataSource={csvData} />
